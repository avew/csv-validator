...
public class CsvewSftpParserTest extends CsvewReader<CsvSftpValue> {
    ...
    public void testReadSuccess() {
        ...
        CsvewResultReader<CsvSftpValue> read = process(1, is);
        System.out.println(read.getValues().toString());
    }


    @Override
    public CsvewResultReader<CsvSftpValue> process(int start, InputStream is) {
        CsvewParser csvParseUser = new CsvewParser();
        return read(
                start,
                is,
                HEADER,
                (int line, String[] columns, Collection<CsvewValidationDTO> validations, CsvSftpValue value) -> parse(line, columns, validations, value, csvParseUser));
    }

    private static void parse(int line, String[] columns, Collection<CsvewValidationDTO> validations, CsvSftpValue value, CsvewParser csvParseUser) throws Exception {
